...
$tIENDA_idTIENDA=$productos->getTIENDA_idTIENDA()->getIdTIENDA();
  ...
  public function select($productos){
      ...
      try {
          $sql= "SELECT `idPRODUCTOS`, `PROVEEDORES_idPROVEEDORES`, `NOMBRE_PRODUCTO`, `UNIDAD_PRODUCTO`, `COSTO_PRODUCTO`, `CANTIDAD_PRODUCTO`, `PRECIOVENTA_PRODUCTO`, `VALORMINIMO_PRODUCTO`, `FECHAINGRESO_PRODUCTO`, `FECHAVENCIMIENTO_PRODUCTO`, `CATEGORIA_idCATEGORIA`,`NOMBRE_CATEGORIA`, `NOMBREEMPRESA_PROVEEDORES`,`TIENDA_idTIENDA`
         FROM `productos` INNER JOIN `categoria` ON (`productos`.`CATEGORIA_idCATEGORIA` = `categoria`.`idCATEGORIA`) INNER JOIN `proveedores` ON (`productos`.`PROVEEDORES_idPROVEEDORES` = `proveedores`.`idPROVEEDORES`)
