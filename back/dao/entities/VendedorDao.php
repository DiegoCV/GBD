...
$tIENDA_idTIENDA=$vendedor->getTIENDA_idTIENDA()->getIdTIENDA();
  ...
  public function listAll(){
     ...
     public function login($vendedor){
      $uSUARIO=$vendedor->getUSUARIO_VENDEDOR();
      $pASS=$vendedor->getPASS_VENDEDOR();

      $vendedor= new Vendedor();
      try {
          $sql= "SELECT `idVENDEDOR`, `NOMBRE_VENDEDOR`, `USUARIO_VENDEDOR`, `PASS_VENDEDOR`, `TIENDA_idTIENDA` FROM `vendedor`  WHERE `USUARIO_VENDEDOR`='$uSUARIO' AND`PASS_VENDEDOR`='$pASS'";
          $data = $this->ejecutarConsulta($sql);
          for ($i=0; $i < count($data) ; $i++) {
         $vendedor->setIdVENDEDOR($data[$i]['idVENDEDOR']);
          $vendedor->setNOMBRE_VENDEDOR($data[$i]['NOMBRE_VENDEDOR']);
